...
class ProductController extends Controller
{
    ...
    public function update($id, Request $request){
        ...
        if(!empty($product)){
            ...
            $product->product_detail = trim($request->product_detail);
            $product->short_detail = trim($request->short_detail);
            $product->additional_information = trim($request->additional_information);
            $product->delivery_returns = trim($request->delivery_returns);
            $product->status = trim($request->status);
            $product->gender = trim($request->gender);
            $product->save();
